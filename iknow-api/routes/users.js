...
router.get("/", async (req, res) => {
  try {
    ...
    const admin = await pool.query("SELECT is_admin FROM users WHERE id = $1", [
      req.user.id,
    ]);
...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (password) {
      ...
      query =
        "UPDATE users SET username = $1, email = $2, password_hash = $3, is_active = $4, is_admin = $5 WHERE id = $6 RETURNING id, username, email, is_active, is_admin, created_at, last_login";
      params = [username, email, hashedPassword, is_active, is_admin, id];
    } else {
      query =
        "UPDATE users SET username = $1, email = $2, is_active = $3, is_admin = $4 WHERE id = $5 RETURNING id, username, email, is_active, is_admin, created_at, last_login";
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (parseInt(content.rows[0].content_count) > 0) {
      return res
        .status(400)
        .json({ error: "Cannot delete user who has created content" });
    }
